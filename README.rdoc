...
= The global "The"-Object
	...
	use(lib,cb,target,supports): include a lib "lib" (without the extension .js), if it not exists and call the callback, "target" is the object who will supports "support" if the lib is loaded (to indicate if its loaded)
		  on default, the lib is in the same directory as the The.js-Script, "target" is window and supports is the same as "lib"
